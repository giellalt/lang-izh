...
function print_usage() {
    ...
    echo "  -l, --add-langvar   Add env. variable \$GTLANG_izh to login script."
...
if test x$langvar = xlangvar ; then
	if test x$LOGINFILE = x ; then
		...
		if test x$GTLANG_izh = x$LANGDIR ; then
			echo "\$GTLANG_izh already defined."
		else
			# Already defined with a different value:
			if test x$GTLANG_izh != x ; then
				renew=renew
				OLDLANGDIR=$GTLANG_izh
			fi

			# Add the variable to the login script:
			cp $LOGINFILE $LOGINFILE.gtbackup.${DATE}-${TIME}
			echo "export GTLANG_izh=$LANGDIR" >> $LOGINFILE
			source $LOGINFILE

			# Feedback depending on whether it was added or redefined:
			if test x$renew = xrenew ; then
			 echo "The env. variable \$GTLANG_izh has been redefined in"
			 echo "$LOGINFILE. The old value $OLDLANGDIR is no longer in use."
			else
			 echo "The env. variable \$GTLANG_izh has been added to $LOGINFILE."
...
# the current dir:
if   test x$GTLANG_izh = x ; then
	echo "WARNING: The variable \$GTLANG_izh has not been defined. You will not"
	...
	echo "$0 -l"
	echo
elif test x$GTLANG_izh != x$LANGDIR ; then
	echo "WARNING: The variable \$GTLANG_izh has the value:"
	echo "  $GTLANG_izh"
